//
//  XLBFollowDynamicViewController.m
//  xiaolaba
//
...
@interface XLBFollowDynamicViewController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,MainMomentsCellDelegate,XLBErrorViewDelegate,XLBShareViewDelegate>
...
@implementation XLBFollowDynamicViewController
...
- (void)viewDidLoad {
    ...
    //    NSString *path=[docPath stringByAppendingPathComponent:@"mainFollowData.archiver"];
...
- (void)getDataFromServer:(NSInteger)current{
    ...
    NSString *URL = [NSString stringWithFormat:@"%@",kHomeFollowList];
    kWeakSelf(self);
    [[NetWorking network] POST:URL params:dict cache:NO success:^(id result) {
        NSLog(@"-------------------  首页--%@  %@  %@",dict,[result objectForKey:@"next"],result[@"offset"]);
        ...
        if (current == 1 && listArr.count == 0) {
            ...
            if (_curr ==1||weakSelf.data.count==0) {
                if (listArr.count!=0) {
                    [weakSelf.data removeAllObjects];
                    [weakSelf.littleModel.imgs removeAllObjects];
                    weakSelf.data = [LittleHornTableViewModel mj_objectArrayWithKeyValuesArray:listArr];
                }
                [self.tableView reloadData];
            }else {
                NSInteger count = weakSelf.data.count-1;
                [weakSelf.data addObjectsFromArray:[LittleHornTableViewModel mj_objectArrayWithKeyValuesArray:listArr]];
                [weakSelf.tableView reloadData];
