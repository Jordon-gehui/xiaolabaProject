...
@implementation BQLChineseString
@synthesize string;
@synthesize pinYin;
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr
{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((BQLChineseString *)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr
{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((BQLChineseString *)object).pinYin substringToIndex:1];
        NSString *string = ((BQLChineseString *)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    {
        BQLChineseString *chineseString = [[BQLChineseString alloc] init];
        ...
        chineseString.string = [BQLChineseString RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)SortArray:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++){
        [result addObject:((BQLChineseString*)[tempArray objectAtIndex:i]).string];
        NSLog(@"SortArray----->%@",((BQLChineseString*)[tempArray objectAtIndex:i]).string);
    }
    return result;
}
