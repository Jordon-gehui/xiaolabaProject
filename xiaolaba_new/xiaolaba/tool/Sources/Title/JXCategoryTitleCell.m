...
- (void)initializeViews
{
    ...
    NSLayoutConstraint *titleLabelCenterY = [NSLayoutConstraint constraintWithItem:self.titleLabel attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeBottom multiplier:1 constant:5];
...
- (void)reloadData:(JXCategoryBaseCellModel *)cellModel {
    ...
    if (myCellModel.titleLabelZoomEnabled) {
        ...
        if (myCellModel.selectedAnimationEnabled && [self checkCanStartSelectedAnimation:myCellModel]) {
            ...
            NSLog(@"font%f",baseScale*myCellModel.titleLabelCurrentZoomScale);
        }
    }else {
